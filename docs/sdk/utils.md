...
2. address ("0x0000000000000000000000000000000000000000" for "ETH" or "0x2e055eee18284513b993db7568a592679ab13188" for
   ERC20).

To use the token-related utility functions, `TokenSet` class must be used, token set can be obtained from
[zkTube provider](providers.md#current-token-set).
...
1. Committed (with `awaitReceipt`) when the state is updated in the zkTube network
...
const wallet = ..; // create zkTube Wallet

// see transfer example for details
const transfer = await wallet.syncTransfer({..});

// this function will return when deposit is committed to the zkTube chain
...
const deposit = await zktube.depositFromETH({..});
...
```typescript
import * as zktube from "zktube-soft-launch";
const provider = ..; // create zkTube Provider
const wallet = ..; // create zkTube Wallet

// sign transfer transaction
const signedTransfer = await wallet.signSyncTransfer({..});

// submit transaction to zkTube
const transfer =  await zktube.wallet.submitSignedTransaction(signedTransfer, provider);
