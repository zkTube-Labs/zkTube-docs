...
// Addresses of the zkTube contracts
...
// Committed nonce is going to be resolved to last nonce known to the zkTube network
...
// 'ECDSA' if it's done by providing an Ethereum signature in zkTube transaction.
// 'CREATE2' if it's done by providing arguments to restore account ethereum address according to CREATE2 specification.
// 'ECDSALegacyMessage' if it's done by providing an Ethereum signature in zkTube transaction. Unlike the 'ECDSA', the user signs a human-readable message. Thus, the fee is ~30% higher than ECDSA.
...
export interface LegacyChangePubKeyFee {
  ChangePubKey: {
    ...
    // 'false' if it's done by providing an Ethereum signature in zkTube transaction.
...
export interface AccountState {
  ...
  // not yet processed by the zkTube network.
  depositing: {
    balances: {
      ...
      [token: string]: {
        ...
        // to be received by zkTube network.
        expectedAcceptBlock: number;
      };
    };
  };
  // Committed state is the last state known to the zkTube network, can be ahead of verified state
  committed: {
    balances: {
      // Token are indexed by their symbol (e.g. "ETH")
...
which still do not have enough confirmations to be processed by the `zkTube` network.
