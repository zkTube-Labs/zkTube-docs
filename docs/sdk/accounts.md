...
[transfer]: #transfer-in-the-zktube
[withdraw]: #withdraw-token-from-the-zktube
...
If not set, lowest possible fee will be requested from zkTube server. Fees are pain in the same token as the main
...
`Wallet` object is used to interact with the zkTube network. The wallet has an ethereum address associated with it and
user that owns this ethereum account owns a corresponding zkTube account. By ownership of ethereum account we mean
...
To create transactions in the zkTube network wallet must have zkTube key pair associated with it. zkTube keys are
...
For zkTube keys to be valid user should register them once in the zkTube network using [set signing key transaction].
For ethereum wallets that do not support message signing [additional ethereum transaction] is required. zkTube keys can
be changed at any time.

Transactions such as [Transfer] and [Withdraw] are additionally signed using ethereum account of the wallet, this
signature is used for additional security in case zkTube keys of the wallet are compromised. User is asked to sign
readable representation of the transaction and signature check is performed when transaction is submitted to the zkTube.
...
| provider                    | zkTube provider that is used for submitting a transaction to the zkTube network. |
| signer (optional)           | zkTube signer that will be used for transaction signing.[^signer]                |
| accountId (optional)        | zkTube account id.[^acc_id]                                                      |
| ethSignatureType (optional) | Type of signature that is produced by `ethWallet`.[^ethsig]                      |
| returns                     | `zktube.Wallet` derived from ethereum wallet (`ethers.Signer`)                   |
...
### Creating wallet from ETH without zkTube signer
...
This way wallet won't contain any valid zkTube keys to perform transactions, but some of the operations can be used
...
const ethWallet = ethers.Wallet.createRandom().connect(ethersProvider);
const syncWallet = await zktube.Wallet.fromEthSignerNoKeys(ethWallet, syncProvider);
```
...
| Name                 | Description                                                                      |
| -------------------- | -------------------------------------------------------------------------------- |
| syncSigner           | zkTube signer that will be used for transaction signing.[^signer]                |
| provider             | zkTube provider that is used for submitting a transaction to the zkTube network. |
...
const syncProvider = await zktube.getDefaultProvider("rinkeby");
const signer = await zktube.Signer.fromSeed(ethers.utils.randomBytes(32));
...
const syncWallet = await zktube.Wallet.fromCreate2Data(signer, syncProvider, create2Data);
```

### Creating wallet from L2 Wallets

Starting from `0.0.12` version of our SDK we support `remote json rpc signer`. That means you could add support of zkTube L2 Wallets such as Argent zkTube or other applications into your dapp. 
...
| Name                 | Description                                                                               |
| -------------------- | ------------------------------------------------------------------------------------------|
| web3Provider         | A provider that is connected to an appropriate wallet, such as Argent zkTube L2 Wallet.   |
| provider             | zkTube provider that is used for submitting a transaction to the zkTube network.          |
| accountId (optional) | zkTube account id.[^acc_id]                                                               |
...
| Name    | Description                                                    |
| ------- | -------------------------------------------------------------- |
| returns | Numerical account ID in the the zkTube tree state.[^undefined] |
...
const erc20VerifiedBalance = await getBalance("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2", "verified");
```

### Get token balance on Ethereum

Method similar to [`syncWallet.getBalance`](#get-token-balance-on-zktube), used to query balance in the Ethereum
network.
...
// Setup zktube.Wallet with ethers signer/wallet that is connected to ethers provider
const wallet = ..;
...
For convenience, it is possible to approve the maximum possible amount of ERC20 token deposits for the zkTube contract
...
Moves funds from the ethereum account to the zkTube account.
...
[provider docs] for exact number) before accepting it in the zkTube network. After the transaction is committed to the
zkTube network, funds are already usable by the recipient, meaning that there is no need to wait for verification before
proceeding unless additional confirmation is required for your application. To wait for the transaction commitment on
the zkTube network, use `awaitReceipt`(see [utils]).
...
| Name                                            | Description                                                        |
| ----------------------------------------------- | ------------------------------------------------------------------ |
| deposit.depositTo                               | zkTube account address of the receiver                             |
...
const syncWallet = ..; // Setup zktube wallet from ethers.Signer.

const depositPriorityOperation = await syncWallet.depositToSyncFromEthereum({
  depositTo: "0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd",
  token: "ETH",
...
In order to send zkTube transactions (transfer and withdraw) user has to associate zktube key pair with account. Every
zkTube account has address which is ethereum address of the owner.

There are two ways to authorize zktube key pair.

1. Using ethereum signature of specific message. This way is preferred but can only be used if your ethereum wallet can
   sign messages.
2. Using ethereum transaction to zkTube smart-contract.

<aside class = notice>
The account should be present in the zkTube network in order to set a signing key for it.
</aside>

This function will throw an error if the account is not present in the zkTube network. Check the
[account id](#get-account-state) to see if account is present in the zkTube state tree.
...
const wallet = ..;// setup zktube wallet
...
Signs [change public key](#changing-account-public-key) transaction without sending it to the zkTube network. It is
...
### Transfer in the zkTube

Moves funds between accounts inside the zkTube network. Sender account should have correct public key set before sending
...
| Name                           | Description                                                                 |
| ------------------------------ | --------------------------------------------------------------------------- |
| transfer.to                    | zkTube address of the recipient of funds                                    |
...
const transferTransaction = await wallet.syncTransfer({
  to: "0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd",
  token: "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2", // FAU ERC20 token address
  amount: ethers.utils.parseEther("1.0"),
  fee: ethers.utils.parseEther("0.001")
});
...
### Sign transfer in the zkTube transaction

Signs [transfer](#transfer-in-the-zktube) transaction without sending it to the zkTube network. It is important to
...
Performs an atomic swap between 2 existing accounts in the zkTube network. For information about swaps, see the
...
const orderB = await walletB.getOrder({
  ...
  recipient: '0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd'
});
...
### Batched Transfers in the zkTube
...
For details on an individual transaction, see [Transfer in the zkTube](#transfer-in-the-zktube).
...
### Withdraw token from the zkTube

Moves funds from the zkTube account to ethereum address. Sender account should have correct public key set before
...
const withdrawTransaction = await wallet.withdrawFromSyncToEthereum({
  ethAddress: "0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd",
  token: "FAU",
  amount: ethers.utils.parseEther("1.0"),
  fee: ethers.utils.parseEther("0.001")
});
...
### Sign withdraw token from the zkTube transaction

Signs [withdraw](#withdraw-token-from-the-zktube) transaction without sending it to the zkTube network. It is important
...
exist more than 24 hours. After execution of the transaction, funds will be transferred from the target zkTube wallet to
...
const forcedExitTransaction = await wallet.syncForcedExit({
  target: "0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd",
  token: "FAU",
  fee: ethers.utils.parseEther("0.001")
});
...
Signs [forced exit](#initiate-a-forced-exit-for-an-account) transaction without sending it to the zkTube network. It is
important to consider transaction fee in advance because transaction can become invalid if token price changes.
...
| Name              | Description                                                     |
| ----------------- | --------------------------------------------------------------- |
| forcedExit.target | zkTube address of the target account                            |
| forcedExit.token  | Token to be transferred[^token].                                |
...
If ordinary withdraw from zkTube account is ignored by network operators user could create an emergency withdraw request
using special Ethereum transaction, this withdraw request can't be ignored.

Moves the full amount of the given token from the zkTube account to the Ethereum account.

Once the operation is committed to the Ethereum network, we have to wait for a certain amount of confirmations (see
[provider docs](providers.md#get-amount-of-confirmations-required-for-priority-operations) for exact number) before
accepting it in the zkTube network. Operation will be processed within the zkTube network as soon as the required amount
...
const syncWallet = ..; // Setup zktube wallet.
...
Two factor authentification is an additional protection layer enforced by zkTube server.
...
const syncWallet = ..; // Setup zktube wallet with private key or EIP-1271 signing
...
Calls the `withdrawPendingBalance` function on the zkTube smart contract. This function is typically used to withdraw
...
const withdrawPendingTx = await syncWallet.withdrawPendingBalance(
  "0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd",
  "ETH",
...
Calls the `withdrawPendingBalance` function multiple times on the zkTube smart contract. To optimize gas usage, instead
...
const withdrawPendingTx = await syncWallet.withdrawPendingBalances(
  ["0x3DFE26Cc740CAb2acBB49Cf118DD44aC2CA8aeCd", "0x2e055eee18284513b993db7568a592679ab13188"],
  ["ETH", "DAI"],
  {},
...
sign the message only once for the whole batch. 
<!-- You can read more about transaction batches
[here](/dev/payments/sending_transactions#sending-transaction-batches). -->

Batch Builder supports all kinds of zkTube L2 transactions, such as: `Withdraw`, `Transfer`, `ChangePubKey`, etc.
...
batchBuilder.addTransfer({
  to: "0x43085Ac677366bD8C114DF35A41BF8266AA2142a",
  token: "ETH",
  amount: ethers.utils.parseEther("0.001")
});
```
...
Signs transfer transaction, the result can be submitted to the zkTube network.
...
| returns            | Signed zkTube transfer transaction      |

### Sign zkTube Withdraw

Signs withdraw transaction, the result can be submitted to the zkTube network.
...
| returns             | Signed zkTube withdraw transaction        |

### Sign zkTube Forced Exit

Signs forced exit transaction, the result can be submitted to the zkTube network.
...
| returns                       | Signed zkTube forced exit transaction     |

### Sign zkTube ChangePubKey

Signs ChangePubKey transaction, the result can be submitted to the zkTube network.
...
| changePubKey.account                 | zkTube address of the account                                               |
...
| returns                              | Signed zkTube change public key transaction                                 |
